...
session_start();
function code($fontSize=16,$length=4,$width=75,$height=25){
    $strNum = GetVerify($length);									//获取一个随机字符串
    $_SESSION['verify'] = $strNum;									//付值给session
    $im = imagecreate($width,$height);								//生成一张指定宽高的图片
    $backgroundcolor = imagecolorallocate ($im, rand(100,255), rand(100,255), rand(100,255));		//生成背景色
    $frameColor = imageColorAllocate($im, 255, 255, 255);			//生成边框色
    $font = ROOT."data/arial.ttf";						            //提取字体文件，开始写字
    for($i = 0; $i < $length; $i++) {
	   $charY = ($height+9)/2 + rand(-1,1);							//定义字符Y坐标
	   $charX = $i*15+8;											//定义字符X坐标											
	   $text_color = imagecolorallocate($im, mt_rand(0, 255), mt_rand(0, 100), mt_rand(0, 100));    //生成字符颜色
	   $angle = rand(-20,20);										//生成字符角度														//写入字符
