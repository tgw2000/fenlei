...
class mysql {
     ...
     * 取一个字段
     */
    function getOne($sql, $limited = false,$flag=0,$ttl=600)
    {
        if ($limited == true) $sql = trim($sql . ' LIMIT 1');
        if($flag){      //打开缓存
            $cache = load_class('cache_file');
            $cache->set_dir(ROOT.'data/cache/sqlcache/');
        ...
        if(!$data){     //不存在缓存，查询数据库
            ...
            if ($res !== false){
                ...
                if ($row !== false){
                    $data = $row[0];
                    if($flag) $cache->set(md5($sql),$data,$ttl);  //写入缓存
                    return $data;
                }else{
                    return '';
                }
     ...
     * $ttl:过期时间，为0表示永不过期
     */
    function getAll($sql,$flag=0,$ttl=600){
     ...
     * 取一条记录
     */
    function getRow($sql,$limited = false,$flag=0,$ttl=600)
    {
        if ($limited == true) $sql = trim($sql . ' LIMIT 1');
 	...
 	function getCol($sql,$flag=0,$ttl=600)
    {
         if($flag){      //打开缓存
            $cache = load_class('cache_file');
            $cache->set_dir(ROOT.'data/cache/sqlcache/');
