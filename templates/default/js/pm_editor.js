...
function insertunit(text, textend, moveend) {
	...
	if(!isUndefined($$('message').selectionStart)) {
		var opn = $$('message').selectionStart + 0;
		if(textend != '') {
			text = text + $$('message').value.substring($$('message').selectionStart, $$('message').selectionEnd) + textend;
		}
		$$('message').value = $$('message').value.substr(0, $$('message').selectionStart) + text + $$('message').value.substr($$('message').selectionEnd);
		if(!moveend) {
			$$('message').selectionStart = opn + strlen(text) - endlen;
			$$('message').selectionEnd = opn + strlen(text) - endlen;
		}
...
function getSel() {
	if(!isUndefined($$('message').selectionStart)) {
		return $$('message').value.substr($$('message').selectionStart, $$('message').selectionEnd - $$('message').selectionStart);
	} else if(document.selection && document.selection.createRange) {
